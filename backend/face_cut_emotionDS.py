...
def faceDetection(emotion):
    files = gb.glob("E:\\projectImplementation\\projectFiles\\masteremotion\\selected_set\\%s\\*" % emotion)  # Get list of all images with emotion
    filenumber = 0
    for f in files:
        ...
        for (x, y, w, h) in facefeatures:  # get coordinates and size of rectangle containing face
            ...
            try:
                ...
                print("wrote in %s file %s"%(emotion, filenumber))
                cv2.imwrite("E:\\projectImplementation\\projectFiles\\final_dataset\\%s\\%s.jpg" % (emotion, filenumber), out)  # Write image
            except:
                print("error")
